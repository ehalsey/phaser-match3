...
export class EndLevelScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // Game Over title
    this.add.text(centerX, centerY - 180, 'Level Complete!', {
      fontSize: '56px',
      color: '#2ecc71',
      fontStyle: 'bold'
    }).setOrigin(0.5);
    ...
    // Score display
    this.add.text(centerX, centerY - 80, 'Final Score', {
      fontSize: '28px',
      color: '#ecf0f1'
    }).setOrigin(0.5);

    this.add.text(centerX, centerY - 30, this.finalScore.toString(), {
      fontSize: '64px',
      color: '#f1c40f',
      fontStyle: 'bold'
    }).setOrigin(0.5);
    ...
    // Play Again button
    const playAgainButton = this.add.rectangle(centerX, centerY + 80, 250, 70, 0x3498db);
    playAgainButton.setInteractive({ useHandCursor: true });

    const playAgainText = this.add.text(centerX, centerY + 80, 'Play Again', {
      fontSize: '28px',
      color: '#ffffff',
      fontStyle: 'bold'
    }).setOrigin(0.5);
    ...
    // Main Menu button
    const menuButton = this.add.rectangle(centerX, centerY + 170, 250, 70, 0x95a5a6);
